...
class Agent():
    ...
    def step(self, state, action, action_second_player, reward, next_state, next_state_second_player, done, timestep = 1):
    ...
    def act(self, state, add_noise=True):
        ...
        if add_noise:
            action += self.noise_factor * self.noise.sample().reshape((-1,2))
    ...
    def learn(self, experiences, gamma):
        ...
        actions_next_second_player = self.actor_target(next_states_other_player)
        ...
        # ------ __new__()----------------- update target networks ----------------------- #
    ...
    def soft_update(self, local_model, target_model, tau):
        ...
        """
        for targ __new__()et_param, local_param in zip(target_model.parameters(), local_model.parameters()):
...
class ReplayBuffer:
    ...
    def __init__(self, action_size, buffer_size, batch_size, seed):
        ...
        self.experience = namedtuple("Experience", field_names=["state", "action", "action_second_player" "reward", "next_state", "next_state_second_player", "done"])
        self.seed = random.seed(seed)
    ...
    def sample(self):
        ...
        actions_second_player = torch.from_numpy(np.vstack([e.action_second_player for e in experiences if e is not None])).float().to(device)
        rewards = torch.from_numpy(np.vstack([e.reward for e in experiences if e is not None])).float().to(device)
        next_states = torch.from_numpy(np.vstack([e.next_state for e in experiences if e is not None])).float().to(device)
        next_states_second_player = torch.from_numpy(np.vstack([e.next_state_second_player for e in experiences if e is not None])).float().to(device)
